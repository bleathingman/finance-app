      ...
      <div class="card">
        ...
        <div v-else class="recurring-list">
          <div v-for="d in depensesRecurrentes" :key="d.id" class="recurring-item">
            ...
            <div class="amount-negative" style="margin-left:auto">-{{ formatAmount(d.montant) }}</div>
            <div style="display:flex;gap:4px;flex-shrink:0">
              <button class="icon-btn" @click="ouvrirEdition(d)" title="Modifier">
              ...
              <button class="icon-btn danger" @click="financeStore.supprimerDepense(d.id)" title="Supprimer">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none">
        ...
        <div v-for="depense in depensesFiltrees" :key="depense.id" class="tx-row">
          ...
          <div class="tx-main">
            ...
            <div class="tx-meta">
              ...
              <span v-for="tag in (depense.tags || [])" :key="tag" class="tag-chip" @click="setFiltreTag(tag)" :class="{ active: filtreTag === tag }" title="Filtrer par ce tag">#{{ tag }}</span>
...
const depensesFiltrees = computed(() =>
  txDuMois.value.filter(d => {
    ...
    if (filtreTag.value && !(d.tags || []).includes(filtreTag.value)) return false
    if (recherche.value) {
...
const allTags = computed(() => {
  const set = new Set()
  txDuMois.value.forEach(d => (d.tags || []).forEach(t => set.add(t)))
  return [...set].sort()
})
...
function ouvrirEdition(depense) {
  ...
  form.value = {
    ...
    tags:        depense.tags ? [...depense.tags] : []
  }
  showModal.value = true
}
function closeModal() {
...
async function handleSauvegarder() {
  ...
  try {
    if (editMode.value && editId.value) {
      await financeStore.modifierDepense(editId.value, { ...form.value, tags: form.value.tags || [] })
    } else {
      await financeStore.ajouterDepense({ ...form.value, tags: form.value.tags || [], compteId: form.value.compteId || null })
    }
    closeModal()
